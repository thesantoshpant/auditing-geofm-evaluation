...
The Wilcoxon JSON is a retained one-off aggregate. Its 12 per-chunk prediction
arrays were not retained, so it is checksummed for provenance but is not
regenerated by `scripts/reproduce_all.sh`. The ten-seed region means support the
India exception independently.
