...
"""
import sys, json, glob, os, numpy as np, rasterio
...
for C in ["india","kenya","vietnam","france","netherlands"]:
    ...
    s2={os.path.basename(f).rsplit("_s2.tif",1)[0]:f for f in glob.glob(f"data/chips_ftw_{C}/*/*_s2.tif")}
    wcf={"_".join(os.path.basename(f).split("_")[:3]):f for f in glob.glob(f"data/chips_ftw_{C}/*/*_worldcover.tif")}
