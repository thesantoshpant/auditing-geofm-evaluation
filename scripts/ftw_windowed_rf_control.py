...
import json, glob, os, numpy as np, pandas as pd, rasterio
...
for C in ["india","kenya","vietnam","france","netherlands"]:
    ...
    s2f={os.path.basename(f).rsplit("_s2.tif",1)[0]:f for f in glob.glob(f"data/chips_ftw_{C}/*/*_s2.tif")}
    K=2  # 5x5
    ...
    j=json.load(open(f"data/results/ftw_controlled_label_comparison_{C}.json"))["models"]
