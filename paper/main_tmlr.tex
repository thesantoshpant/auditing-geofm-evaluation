...
\author{
\name Santosh Pant\thanks{OpenAI Codex and Anthropic Claude were used for language editing, software debugging, and consistency checks. The scientific contributions, results, and final decisions are the author's own. The author independently verified the experiments, analysis, code, citations, claims, and manuscript and takes full responsibility for the work.}
...
between-model gap interpreted substantively elsewhere in the paper (about $0.04$ for the
...
Cambodia); India is the certified GeoFM exception (Wilcoxon $p{=}2.4\times10^{-4}$
...
Prithvi leads by roughly 0.04 AUROC, and the direction is supported by a separate
...
Cambodia, by contrast, has the smallest fields (79\% under 0.3 ha) but the highest
positive-pixel coverage (23\%) and does not show the same failure. Parcel size alone is
therefore insufficient to explain the result. Kenya has 155 training chips, whereas
...
Prithvi full fine-tuning, about $110\times$ fewer. Across both GeoFMs, frozen decoders match
AUROC in the 9 of 12 TOST-equivalent region$\times$model cells
(Table~\ref{tab:paired_tost}) and
placing TerraMind India essentially at the equivalence margin by point estimate
($\Delta{=}{+}0.020$) but not by CI. Kenya is the only region where full-FT
...
the same audit in other GeoFM evaluations.
...
This appendix collects the reproducibility details and sensitivity controls referenced from the main text: the per-model frozen-probe breakdown (Table~\ref{tab:fm_breakdown}), seed-paired TOST detail (Table~\ref{tab:paired_tost}) and its $\varepsilon$-sensitivity sweep (Table~\ref{tab:eps_sweep}), the training-recipe key (Table~\ref{tab:recipe}), the full cross-region transfer matrix (Table~\ref{tab:xfer-full}) and its AP/IoU companion (Table~\ref{tab:xfer_metrics}), U-Net epoch sensitivity (Table~\ref{tab:unet_epoch_sensitivity}) and Kenya convergence trajectories (Figure~\ref{fig:convergence}), the per-region operational regime (Table~\ref{tab:regional_regime}), the spatial-pooling control (Table~\ref{tab:windowed}), the FTW partial-labeling sensitivity (Table~\ref{tab:partial}), the tile-disjoint split sensitivity (Table~\ref{tab:tilesplit}), and the full threshold-metric breakdown (Table~\ref{tab:xmetrics}). All numbers are recomputed from the released artifact.
