...
frozen models. On AUROC and average precision (AP), the U-Net leads the full 30-transfer matrix, while an exploratory analysis
...
Label choice changes the benchmark before adaptation enters the comparison. WorldCover
inflates the spectral-baseline score without changing the imagery, whereas polygon-derived
field labels expose the need for a spatial baseline. On those labels, the U-Net has higher
point AUROC than the Prithvi frozen probe in most regions, with India as the exception. PANGAEA
\citep{pangaea} reports a similar supervised-baseline pattern on AI4SmallFarms. The
contrasting comparison in the Prithvi-EO-2.0 report uses multi-temporal crop segmentation
\citep{prithvi} and therefore addresses a different task.

The adaptation comparison also changes once decoder capacity is matched. Frozen and fully
fine-tuned backbones usually agree in-region; Kenya's sparse-positive setting is the shared
failure case. Cross-region transfer is metric-dependent. On AUROC and AP, the U-Net leads
the full 30-transfer matrix. On AUROC, frozen-over-fine-tuned gains appear only in the
exploratory non-Kenya subset. The practical
appeal of a frozen decoder therefore comes from low training cost and frequent in-region
parity. The transfer results do not support a universal advantage.
...
pair the same decoder (2.75M parameters for Prithvi; 2.16M for TerraMind). This
capacity-matched comparison tests whether a reported fine-tuning gain comes from
backbone adaptation or from giving the fine-tuned model a stronger decoder. With
decoder capacity matched, the difference falls within the
declared equivalence margin in 9 of 12 cells (Section~\ref{sec:spine}).

The available labels introduce a second geospatial question that is not captured by
the in-distribution/out-of-distribution framing in \citet{kumar2022finetuning}: what
task does the benchmark measure, and what happens when positives are exceptionally
sparse? On the same pixels,
...
``cropland'' (class~40, \citealp{worldcover}). The task throughout is per-pixel binary
...
and which tables report each configuration. Decoder counts use Prithvi/TerraMind order.}
\label{tab:configs}
\begin{tabular}{lrcll}
\toprule
...
we report mean and a seed-paired $t$-interval with $\mathrm{df}=9$ for matched
frozen-decoder vs.\ full-FT differences (the same paired analysis used in
Table~\ref{tab:spine} and Appendix Table~\ref{tab:paired_tost}). Frozen-probe AUROC in
Table~\ref{tab:flaws} carries chip-level 95\% bootstrap CIs ($B{=}1000$) recomputed
...
not as a universal standard for field mapping. It is less than half of the India
frozen-probe-over-U-Net gap ($0.042$) that we interpret substantively in Section~\ref{sec:flaws},
and more than twice the largest per-configuration seed
...
positive). The evaluation code, metric artifacts, and reproduction commands are
released. Primary aggregates recompute from the released run files, and direct
raster checks verify label-to-image alignment (Appendix~\ref{app:repro}).
...
reflects the choice of baseline. The Prithvi frozen-probe column reports a single
...
$t$-interval ($\mathrm{df}=9$) for ten-seed U-Net and Prithvi full-FT. The full-FT
column is end-to-end fine-tuned Prithvi. Per-model tile-disjoint sensitivity is in
...
\toprule
Region & RF \textsc{true} & RF \textsc{proxy} & U-Net & Prithvi frozen-probe & Prithvi full-FT \\
\midrule
...
property. Across the five regions with shipped coverage, WorldCover labels 22--70\% of
pixels as cropland, versus 0.1--18\% true field coverage. The proxy therefore asks ``is this managed land?''
instead of ``is this pixel in a field?''
...
the gap (with a small drop on Kenya; Table~\ref{tab:windowed}). The per-region best across \{Prithvi, TerraMind, Clay, AnySat\}
...
positive). The chunk audit shows that India is stable across test chunks rather than
...
between-region scale. The released alignment summary gives Cambodia a smaller
mean polygon area than India (0.20 vs.\ 0.36 ha), but the U-Net
leads its frozen probe (0.948 vs.\ 0.924), so field size alone does not explain the pattern. We do not have a
single confident explanation. The India exception cautions against treating the
supervised-baseline result as universal; the field-size comparisons above do not identify a mechanism.
...
Together with the 150-epoch convergence diagnostic (Appendix
Figure~\ref{fig:convergence}), these controls do not indicate that the U-Net result
is an artifact of under-training the GeoFMs.
...
segmentation task rather than a polygon-derived field-extent benchmark. Both evaluation choices appear in the published literature,
...
\label{sec:spine}
Except for the frozen-probe boundary-zone control in Table~\ref{tab:edgezone}, all GeoFM
comparisons in this section use the frozen-decoder and full-FT configurations with matched training seeds; see Table~\ref{tab:configs}.
...
($\mathrm{df}=9$). A dagger marks TOST-equivalence at $\varepsilon{=}0.02$ AUROC by the
...
performance is statistically equivalent to full fine-tuning in 9 of 12
...
Table~\ref{tab:flaws}, we re-evaluated the frozen-probe feature classifiers using a
boundary-zone pixel pool within $\le 2$ pixels of a field edge over the India chips
($n{=}203{,}864$), split 75/25 by chip. AUROC is computed only on the held-out split. The five classifiers evaluated here (four frozen probes $+$ the per-pixel
...
\begin{table}[H]\centering\small
\caption{\textbf{Frozen-probe boundary-zone evaluation (India).} A pool of
$n{=}203{,}864$ pixels within $\le 2$ pixels of a field edge is split 75/25 by chip,
and AUROC is computed only on the held-out split. The five classifiers evaluated here
(four frozen probes $+$ the per-pixel RF) all fall to near-chance AUROC, against
0.94--0.98 on the full-region task for the three transformer-based FMs.
...
pixels. None of the four frozen-probe configurations is a boundary-delineation
...
\midrule
boundary zone ($\le$2 px) & 0.586 & 0.571 & 0.576 & 0.584 & 0.582 & see row below \\
...
\paragraph{Cross-region.} For cross-country transfer without target-region
...
The non-Kenya subset was not pre-registered; we defined it after observing Kenya's
in-region behavior and treat it as exploratory. The full 30-transfer matrix remains
the confirmatory cross-region analysis.
Kenya is
...
Under this measured combination, frozen-feature decoders are unstable (seed standard
deviation up to $0.066$ AUROC), and the supervised U-Net is the most robust tested model on
threshold metrics.
...
(mean IoU 0.086). The Kenya failure appears in both AUROC and threshold metrics,
so it is not an artifact of relying on AUROC alone.
...
failure. Reducing its full-fine-tuning set to 11, 27, or 54 chips, all far below Kenya's
155, leaves AUROC at
...
region$\times$model cells (Table~\ref{tab:paired_tost}). For Prithvi, the frozen
decoder trains a small fraction of the parameters used by full fine-tuning. GPU-minute entries are approximate development-run
...
compute cost alongside accuracy. In this study, Kenya is the only region below
approximately 0.2\% positive pixels; this is a descriptive warning regime rather
...
For a new region with no local labels, the exploratory non-Kenya transfer subset
supports frozen decoders over full fine-tuning for both GeoFMs, but the full
cross-region matrix does not support a broad claim
...
partial annotation as the source of the frozen-FM result (Appendix Table~\ref{tab:partial}), but it cannot
rule out every labeling effect. Finally, the cross-region AUROC evidence is deliberately
narrow: the U-Net leads the full 30-transfer matrix, and the statistically supported
...
the full 30-transfer matrix on AUROC and AP. On AUROC, frozen decoders outperform full fine-tuning only in the
...
multi-temporal regimes, where Prithvi-EO-2.0 was designed to operate, are out of scope.
...
over seeds, 95\% paired $t$-interval with $\mathrm{df}=9$, and TOST verdict at declared
...
Table counts use the same 95\% seed-paired CI-containment criterion as the primary verdicts
($\mathrm{df}=9$). At five of the six margins, Holm correction across the 12 TOST
$p$-values retains the same number of cells. At $\varepsilon{=}0.015$, it retains 7
rather than 8 because TerraMind--Netherlands has
$p_{\mathrm{Holm}}{=}0.112$. The failing set is
...
\url{https://github.com/thesantoshpant/auditing-geofm-evaluation} and archived at
\url{https://doi.org/10.5281/zenodo.21993508} (v1.0.0).
...
on the full matrix (0.826). On the exploratory non-Kenya 20 subset, TerraMind frozen-decoder (0.839) is
close to U-Net (0.842) and beats TerraMind full-FT (0.813), while Prithvi frozen-decoder
...
In the compact headers, ``frz'' denotes frozen-decoder.}
\label{tab:xfer-full}
\tablebodygap
\begin{tabular}{lccccc}
\toprule
...
leads the full matrix, and U-Net and TerraMind frozen-decoder differ by only 0.001 AP on the
...
TerraMind frozen-dec. & 0.781 & 0.696 & \textbf{0.353} & 0.839 & 0.804 & \textbf{0.469} \\
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Region & U-Net@150ep & U-Net@80ep & $\Delta$ AUROC & Prithvi frozen-probe \\
\midrule
